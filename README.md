...
- runtime bundle generation, wrapper helpers, direct session-spawn payload preparation, hybrid persistent/ephemeral topology, OpenClaw spawn integration helpers, named-agent/specialist-subagent operating model, named-agent deployment layer, direct named-agent invocation helpers, project-scoped agent namespaces, workspace-bootstrap/session-rollover deployment model, managed named-agent workspace files, project-agent namespace setup tooling, label-driven spec-approval automation boundary, and named-agent routing precedence for deployed archetype sessions
- `repo-templates/` downstream project GitHub templates
- `scripts/` helper scripts for repo bootstrap, agent git identity, standardized issue/PR/wiki creation or updates, and validation/linting
